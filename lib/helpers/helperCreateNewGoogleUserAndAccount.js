/**
 * createNewUser.js
 */

import prisma from "@/lib/prisma"
import { resMessage } from "./helpers"

const helperCreateNewGoogleUserAndAccount = async (credentials) => {


  if (!credentials) {
    return resMessage('Missing required elements to register user!', 409)
  }

  console.log("The fucjinggg", credentials)


  const { given_name, family_name, email, picture, access_token,
    type, provider, expires_at,
    token_type, scope, id_token } = credentials

  const user = await prisma.user.create({
    data: {
      firstName: given_name,
      lastName: family_name,
      email: email,
      image: picture,
      role: 'USER'
    },
  })

  if (!user) console.log("Unable to create new user")
  if (!user) return resMessage('Unable to create new user', 500)

  const account = await prisma.account.create({
    data: {
      userId: user.id,
      type: type,
      provider: provider,
      providerAccountId: user.id,
      access_token: access_token,
      expires_at: expires_at,
      token_type: token_type,
      scope: scope,
      id_token: id_token
    },
  })


  if (user && account) console.log("ALL GOOD")

  if (user && account) return resMessage("All good", 200)


  return resMessage('Unable to link account to created user', 500)
}
export default helperCreateNewGoogleUserAndAccount
